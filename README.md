...
- 矢はRigidbodyによる重力をかけたかったためRaycastではなくInstantiateした。
- 矢が正面へ飛ばない問題があった。Instantiate位置からCanvasをScreenSpace-Cameraに設定し、WorldToScreenPointやScreenPointToRayを使い、目の前にあるCameraCanvasに向かって打つ方式へ。Debug.DrawRayで目視確認しながら調整した。
...
## プログラミング技術（機能・デザイン） 
- SOLIDの原則に基づき、各クラスを単一責任になるよう分離した。
